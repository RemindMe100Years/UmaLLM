...
class Main_Translator:
    ...
    def _looks_like_fragment(self, a, b):
        ...
        a_ends_lowercase = bool(a_end and a_end[-1] in lower_chars)
        b_starts_lowercase = bool(b_start and b_start[0] in lower_chars)
        both_short = len(a_end) < 80 and len(b_start) < 80
        if a_ends_lowercase or (b_starts_lowercase and both_short):
            return True
        return False
    ...
    def _realign_translations(self, input_lines, translations):
        ...
        if m < n:
            out = list(translations) + [translations[-1]] * (n - m)
            return out[:n]
        ...
        while i < m and len(out) < n:
            ...
            if remaining_out == 1:
                merged = " ".join(t.strip() for t in translations[i:])
            ...
            if need_merge or force_merge:
                merged = (" ".join(t.strip() for t in translations[i:i + 2])).strip()
                out.append(merged)
                i += 2
            else:
    ...
    def _extract_json_array(self, raw_response, context_label=""):
        ...
        if not numbered:
            numbered = re.findall(r"^(\d+)\)\s+(.*)$", text, re.MULTILINE)
        if numbered:
            indexed = [(int(idx), val) for idx, val in numbered]
            indexed.sort(key=lambda x: x[0])
            return [val.strip() for _, val in indexed]
    ...
    def _translate_chunk_with_context(self, batch):
        ...
        current_turn_prompt = (
            ...
            + f"\n\nOutput a JSON array of exactly {expected_count} strings, one per line."
        )
        ...
        for attempt in range(self.max_retries):
            final_payload = history + [{"role": "user", "content": current_turn_prompt}]
            result = self.execute(messages=final_payload)
            translations = self._extract_json_array(result, "")
        ...
        if len(translations) != expected_count:
            valid_trans = [t if t.strip() else "" for t in translations[:expected_count + 1]]
    ...
    def _process_batch_llm(self, list_of_text):
        ...
        return [
            plugins.process_output_text(t) for t in translations[: len(processed_input)]
        ]
