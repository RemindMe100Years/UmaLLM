...
Features:
- **Structured output** - Uses JSON schema to constrain LLM responses. Greatly increases the chance of the LLM providing a valid JSON block. Your only enemy now is LLM hallucination.
...
- **Prompt Caching** - Prompt caching implemented and works as long as your provider supports it. Saves a few seconds, but the main benefit is the credits saved when paying for an API.
...
| VRAM | Model |
|:---:|:---:|
| **0-4 GB** | <img width="480" height="240" src="https://github.com/user-attachments/assets/be66dc4c-61d1-4459-b1b8-7d6ba6130388"/> |
| **5-8 GB** | [Gemma 4 E4B](https://huggingface.co/google/gemma-4-E4B) |
| **12 GB** | [Gemma 4 26B A4B](https://huggingface.co/google/gemma-4-26B-A4B-it), partial offload to RAM |
| **16 GB** | [Gemma 4 26B A4B](https://huggingface.co/google/gemma-4-26B-A4B-it), some quants can fit entirely in VRAM |
| **24 GB** | [Gemma 4 26B A4B](https://huggingface.co/google/gemma-4-26B-A4B-it) for speed, [Gemma 4 31B](https://huggingface.co/google/gemma-4-31B-it) for quality |
| **32 GB** | [Gemma 4 26B A4B](https://huggingface.co/google/gemma-4-26B-A4B-it) for speed, [Gemma 4 31B](https://huggingface.co/google/gemma-4-31B-it) for quality |
...
2. Run setup.bat to install dependencies (botocore is optional, LiteLLM seems to require it if you use AWS Bedrock/SageMaker. Jamdict is an optional sanity check that should theoretically improve translation quality)

3. Run configure.bat to configure LLM + Other settings. Default settings points to llama.cpp, with instructions to localize dialogue from Japanese to English
...
4. Configure Hachimi to point to port 14368:
   - Open `Game Root/hachimi/config.json` and set:
     ...
     ```
     **If the device you're playing from is not the same as the one hosting the server, you need to use the host's local IPV4 address**
...
| `context_lines` | How many previous lines to keep in conversation history. Recommend to set this to 0 to minimize hallucinations. (default: `0`) |
...
| `max_tokens` | Max tokens per LLM response. If your translations are getting cut off, increasing this might help (default: `4096`)  |
| `parallel_workers` | **1** = single batch mode (all lines in one call). **>1** = parallel mode (lines split into chunks) (default: `3`) |
| `chunk_size` | Number of lines per chunk when using parallel mode (ignored when `parallel_workers` = 1). Set to `"auto"` to automatically calculate chunk size based on the number of lines and `parallel_workers`. (default: `"auto"`) |
| `auto_min_total_lines` | Minimum total lines to use all workers when `chunk_size` is `"auto"`. If total lines >= this value, all `parallel_workers` are used. If below, only 1 worker handles everything. E.g., 50 lines with min 30 = all workers. 20 lines with min 30 = 1 worker. Helps when prompt preprocessing is the main bottleneck.  (default: `30`) |
| `auto_min_lines_per_worker` | Minimum estimated lines per worker when `chunk_size` is `"auto"`. Checks `total_lines // parallel_workers`. If below this value, only 1 worker is used. Set to `0` to disable. E.g., 31 lines, 3 workers, min 15 = ~10/worker < 15, 1 worker. (default: `15`) |
...
| `append_all_characters` | Only relevant if using parallel TL. If `true`, scans the full batch once. Any characters that matched are added to the current glossary, and that glossary is shared with all chunks. Trade off speed for very minor quality improvement when using parallel translation. (default: `false`) |
...
| `do_horses_exist` | If `false`, automatically fails and retries translated lines containing the word "horse" (and compounds like "horsehoof" and "horseshoe"). Defaults to true to minimize non-essential retries (default: `true`)|
...
| `prompt_cache_enabled` | If `true`, sends the system prompt as a separate message so backends with prompt caching can cache the stable prefix and only re-process variable content. Reduces latency on retries and repeated calls. Set to `false` if your backend doesn't support prompt caching. (default: `true`) |
...
- For an RTX 5090, I've found `parallel_workers: 3`, `chunk_size: auto`, `auto_min_total_lines: 30`, `auto_min_lines_per_worker: 15` to be the sweet spot. Using gemma-4-26b-a4b-it@q4_k_m, this setting usually translates almost every event in 4-10 seconds, and for large walls of text it takes around 15 seconds. The good news is Hachimi caches auto translation, which means you can just set Auto Translate to on and enable Auto Play and read a Trainee's story with minimal interruptions the next time you encounter the same event.
...
I haven't done extensive testing if this actually helps, but theoretically, it should help when you are using an old LLM like [VNTL Llamma](https://huggingface.co/lmg-anon/vntl-llama3-8b-v2-gguf) or a model that is not good at translating from English to Japanese. Additional latency is in the milisecond range.
...
| Script | Purpose |
|---|---|
| server.py | Entry point. Sets up signals, loads config, creates translator, starts Flask app |
| app.py | Flask routes. Handles POST/GET for translation, health check, shutdown |
| pipeline.py | Translation orchestration. Single-line, batch, and parallel chunked workflows with retries and quality checks |
| prompts.py | Prompt building. UID format, message assembly, batch splitting, prompt caching |
| translator.py | Main_Translator class. Config loading, capability probing, dispatches to pipeline |
| quality.py | Quality checks. Trivial output detection, horse check, Jamdict sanity check |
| config.py | Settings/logging. Loads settings.json, character_memory.json, sets up logger with redaction |
| character_memory.py | Character handling. Name matching, honorific detection, memory apply/build |
| parsers.py | Response parsing. UID extraction, JSON parsing, translation cleaning |
| llm_client.py | LLM interface. Capability caching, JSON schema building |
| plugins.py | Text preprocessing. Input filtering, output cleanup |
| test_jamdict.py | Standalone test. Verifies Jamdict installation works |
| save_json.py | CLI utility. Edits character_memory.json from command line |
