...
function Write-Menu {
    ...
    Write-Host "  [7]  Auto Min Total Lines     : $($settings.auto_min_total_lines)"
    Write-Host "  [8]  Auto Min Lines/Worker    : $($settings.auto_min_lines_per_worker)"
    Write-Host "  [9]  Max Retries              : $($settings.max_retries)"
    Write-Host "  [10] Temperature              : $($settings.temperature)"
    Write-Host "  [11] Top P                    : $($settings.top_p)"
    Write-Host "  [12] Top K                    : $($settings.top_k)"
    Write-Host "  [13] Repetition Penalty       : $($settings.repetition_penalty)"
    Write-Host "  [14] Max Tokens               : $($settings.max_tokens)"
    Write-Host "  [15] Min P                    : $($settings.min_p)"
    Write-Host "  [16] Frequency Penalty        : $($settings.frequency_penalty)"
    Write-Host "  [17] Presence Penalty         : $($settings.presence_penalty)"
    Write-Host "  [18] Output Language          : $($settings.output_language)"
    Write-Host "  [19] Trainer Gender           : $(Get-TrainerGender)"
    Write-Host "  [20] Append All Characters    : $($settings.append_all_characters)"
    Write-Host "  [21] Jamdict Sanity Check     : $($settings.jamdict_sanity_check)"
    Write-Host "  [22] Preserve Honorifics      : $($settings.preserve_honorifics)"
    Write-Host "  [23] Prompt Cache             : $($settings.prompt_cache_enabled)"
    Write-Host "  [24] Do Horses Exist          : $($settings.do_horses_exist)"
    Write-Host ""
    Write-Host "  [25] Manage Characters" -ForegroundColor Yellow
    Write-Host ""
...
while (-not $exit) {
    ...
    switch ($choice) {
        ...
        "6" {
            ...
            elseif ($val -and $val.Trim().ToLower() -eq "auto") { $settings.chunk_size = "auto"; Save-Settings }
        }
     "7" {
            $val = Read-Host "  Auto min total lines (current: $($settings.auto_min_total_lines))"
            if ($val -and $val.Trim() -match '^\d+$') { $settings.auto_min_total_lines = [int]$val.Trim(); Save-Settings }
        }
        "8" {
            $val = Read-Host "  Auto min lines per worker (current: $($settings.auto_min_lines_per_worker))"
            if ($val -and $val.Trim() -match '^\d+$') { $settings.auto_min_lines_per_worker = [int]$val.Trim(); Save-Settings }
        }
        "9" {
        ...
        "23" {
            $val = Read-Host "  Prompt cache (current: $($settings.prompt_cache_enabled), y/n)"
            $valLower = $val.Trim().ToLower()
            if ($valLower -eq "y") { $settings.prompt_cache_enabled = $true; Save-Settings }
            elseif ($valLower -eq "n") { $settings.prompt_cache_enabled = $false; Save-Settings }
            elseif ($val -and $val.Trim()) { Write-Host "  Invalid input! Use y or n." -ForegroundColor Red }
        }
        "24" {
            $val = Read-Host "  Do horses exist (current: $($settings.do_horses_exist), y/n)"
            $valLower = $val.Trim().ToLower()
            if ($valLower -eq "y") { $settings.do_horses_exist = $true; Save-Settings }
            elseif ($valLower -eq "n") { $settings.do_horses_exist = $false; Save-Settings }
            elseif ($val -and $val.Trim()) { Write-Host "  Invalid input! Use y or n." -ForegroundColor Red }
        }
        "25" { Manage-Characters }
        default {
            if ($choice -and $choice.Trim()) {
                Write-Host ""
                Write-Host "  Invalid option." -ForegroundColor Red
